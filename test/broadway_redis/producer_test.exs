defmodule BroadwayRedis.ProducerTest do
  use ExUnit.Case
  ...
  defmodule FakeRedisClient do
    @behaviour BroadwayRedis.RedisClient
  ...
  test "raise an ArgumentError with proper message when the redis_instance option is nil" do
    assert_raise(
      ArgumentError,
      "invalid options given to BroadwayRedis.RedixClient.init/1, expected :redis_instance to be a atom, got: nil",
      fn ->
        BroadwayRedis.Producer.init(
          redis_instance: nil,
          list_name: "foo",
  ...
  test "raise an ArgumentError with proper message when the redis_instance option is invalid" do
    assert_raise(
      ArgumentError,
      "invalid options given to BroadwayRedis.RedixClient.init/1, expected :redis_instance to be a atom, got: \"my_redis_instance\"",
      fn ->
        BroadwayRedis.Producer.init(
  ...
  test "raise an ArgumentError with proper message when the list_name option is nil" do
    assert_raise(
      ArgumentError,
      "invalid options given to BroadwayRedis.RedixClient.init/1, expected :list_name to be a non empty string, got: nil",
      fn ->
        BroadwayRedis.Producer.init(
          redis_instance: :my_redis_instance,
          list_name: nil,
  ...
  test "raise an ArgumentError with proper message when the list_name option is empty" do
    assert_raise(
      ArgumentError,
      "invalid options given to BroadwayRedis.RedixClient.init/1, expected :list_name to be a non empty string, got: \"\"",
      fn ->
        BroadwayRedis.Producer.init(
          redis_instance: :my_redis_instance,
          list_name: "",
  ...
  test "raise an ArgumentError with proper message when the working_list_name option is nil" do
    assert_raise(
      ArgumentError,
      "invalid options given to BroadwayRedis.RedixClient.init/1, expected :working_list_name to be a non empty string, got: nil",
      fn ->
        BroadwayRedis.Producer.init(
          redis_instance: :my_redis_instance,
          list_name: "foo",
  ...
  test "raise an ArgumentError with proper message when the working_list_name option is empty" do
    assert_raise(
      ArgumentError,
      "invalid options given to BroadwayRedis.RedixClient.init/1, expected :working_list_name to be a non empty string, got: \"\"",
      fn ->
        BroadwayRedis.Producer.init(
          redis_instance: :my_redis_instance,
          list_name: "foo",
